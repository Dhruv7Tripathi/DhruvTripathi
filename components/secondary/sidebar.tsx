...
export default function Sidebar() {
  ...
  return (
    <>
      <div className="hidden md:flex md:w-60 md:flex-col md:fixed md:inset-y-0 bg-black backdrop-blur-lg border border-white/20 shadow-lg">
            ...
            <nav className="flex-1 flex flex-col items-center justify-center space-y-6">
              {navItems.map((item, index) => (
                <Link
                  key={index}
                  href={item.href}
                  className="flex justify-center items-center p-3 text-white rounded-md hover:bg-white hover:text-white w-full transition duration-200"
                >
